...
const minutes = v => ms(minutes) / ms('1m');
...
async function start (bridge, entries) {
  ...
  let entries = await iterator.read({
    minutes: minutes('1d')
  });
  console.log('Requested one day of readings, got %d', entries.length);
  await Promise.all(entries.map(reading => create(reading)));
  entries = null;
